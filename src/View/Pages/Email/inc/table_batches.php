...
<form method="post">
    <table id="batches" class="table table-sm" style="width:100%" data-page-length='25'>
        ...
        foreach ($batches as $row) { ?>
            <tr>
                ...
                <td><a href="Messages?batch_id=<?= $row['id'] ?>" title="Details" class="btn btn-success btn-sm"><i
    ...
    <?php
    if ($pageType === 'index') { ?>
        <input type="submit" class="btn btn-primary" name="sendBatchById"
               value="<?= Text::get('LABEL_SEND_EMAIL') ?>"><?php
    } ?>
