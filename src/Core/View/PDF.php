...
class PDF
{
    public static bool $defined = false;
    public static $error;

    public static function renderInvoice(object $invoice, array $invoiceitems, object $contract): string
    {
    ...
    public static function config()
    {
        ...
        define('K_THAI_TOPCHARS');
        define('K_TCPDF_CALLS_IN_HTML');
    ...
    {
        $pdf->SetX(20);
        $pdf->Write(0, 'Wij verzoeken u het bedrag binnen 14 dagen over te maken naar', '', 0, '');
        $pdf->SetX(20);
        ...
        $pdf->SetX(20);
        $pdf->Write(0, 'De penningmeester van Poppodium de Beuk.', '', 0, 'L');
        return $pdf;
    }

    public static function setInvoiceContent(TCPDF $pdf, array $invoiceitems = null): TCPDF
    {
    ...
    public static function setInvoiceHeader(TCPDF $pdf, $invoice, $contract): TCPDF
    {
        ...
        $pdf->SetX(20);
        $pdf->Write(0, $contract->bandleider_postcode . ' ' . $contract->bandleider_woonplaats, '', 0, 'L');
        $pdf->SetXY(20, 110);
        $pdf->SetFont('dejavusans', 'B', 11, '');
        $pdf->Write(0, 'Factuur', '', 0, 'L');
        $pdf->SetXY(20, 120);
        $pdf->SetFont('dejavusans', '', 11, '');
        $pdf->Write(0, 'Factuurnummer: ' . $invoice->factuurnummer, '', 0, 'L');
        $pdf->SetXY(120, 120);
        $pdf->Write(0, 'Factuurdatum: ' . date('d-m-Y', strtotime($invoice->factuurdatum)), '', 0, 'R');
        $pdf->SetXY(20, 140);
        return $pdf;
    }
    ...
    public static function writeInvoice(object $invoice, array $invoiceitems, object $contract): bool
    {
        ...
        if (file_exists($_SERVER['DOCUMENT_ROOT'] . 'content/invoices/' . $invoice->factuurnummer . '.pdf')) {
            $this->addFlash('danger','Bestand bestaat al.');
        } else {
            // ob_end_clean();
