...
class InvoiceMapper
{
    ...
    {
        $stmt = DB::conn()->prepare('DELETE FROM invoices WHERE id = ? LIMIT 1');
        $stmt->execute([$id]);
    ...
    {
        $stmt = DB::conn()->prepare('SELECT id FROM invoices WHERE year = ?');
        $stmt->execute([$year]);
    ...
    {
        $stmt = DB::conn()->prepare('UPDATE invoices SET mail_id = ? WHERE id = ? LIMIT 1');
    ...
    {
        $stmt = DB::conn()->prepare('UPDATE invoices SET status = ? WHERE id = ? LIMIT 1');
