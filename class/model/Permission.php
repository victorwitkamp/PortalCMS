...
class Permission
{
    ...
    public static function get($perm_id) {
        $stmt = DB::conn()->prepare("SELECT * FROM permissions WHERE perm_id = ?");
    ...
    public static function assign($role_id, $perm_id) {
        $stmt = DB::conn()->prepare("INSERT INTO role_perm(role_id, perm_id) VALUES (?,?)");
    ...
    public static function unassign($role_id, $perm_id) {
        $stmt = DB::conn()->prepare("DELETE FROM role_perm where role_id=? AND perm_id=? LIMIT 1");
        if (!$stmt->execute([$role_id, $perm_id])) {
            return false;
        }
        return true;
    ...
    public static function getPermissions($user_id) {
        ...
        WHERE t1.role_id in (SELECT role_id from user_role where user_id = ?)"
        );
