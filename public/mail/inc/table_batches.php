    ...
    <table id="batches" class="table table-sm" style="width:100%" data-page-length='25'>
        ...
        <tbody>
            ...
            foreach ($batches as $row) {  ?>
                <tr>
                    <td class="text-center" ><input type="checkbox" name="id[]" id="checkbox" value="<?php echo $row['id']; ?>"/></td>
                    ...
                    <?php if ($pageType === 'history') { ?><td><?php echo $row['DateSent']; ?></td><?php } ?>
                    <td><?php
                    if ($row['status'] === '1') { ?><span class="badge badge-secondary">Klaar voor verzending</span><?php }
                    if ($row['status'] === '2') { ?><span class="badge badge-success">Uitgevoerd</span><?php } ?></td>
                    <td><?php echo $row['CreationDate']; ?></td>
                    <td><a href="messages.php?batch_id=<?php echo $row['id']; ?>" title="Details" class="btn btn-success btn-sm"><i class="fas fa-info"></i></a></td>
                </tr>
            <?php } ?>
        </tbody>
    </table>
