...
<?= $this->push('main-content') ?>
    ...
    <div class="container">
        ...
        if (!empty($this->request->get('batch_id'))) {
            $result = MailScheduleMapper::getByBatchId((int)$this->request->get('batch_id'));
        } else {
        ...
        if ($result) {
            if (!empty($this->request->get('batch_id'))) {
                echo '<h3>Berichten van batch ' . $this->request->get('batch_id') . '</h3>';
            } else {
